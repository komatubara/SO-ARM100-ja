# SO-101 用六角ナット埋め込み手首カメラ（MF）取り付けガイド

<img width="300" src="https://github.com/user-attachments/assets/ef919565-230d-43b4-8363-feb26f23967c" />
<img width="300" src="https://github.com/user-attachments/assets/ae01ce6a-a953-4787-b792-7a17e8091094" />

## 概要
このガイドでは、**カメラモジュール** と 3D プリントした **六角ナット埋め込みカメラアダプタ** を使い、SO-101 に **手首カメラ**（別設計の "McGartoll-Fung" 版）を取り付ける手順を説明します。

[Wrist Camera (MF) UVC Module](../Wrist_Cam_Mount_32x32_UVC_Module/) と [Wrist Camera (MF) UVC Plug‑on Module](../Wrist_Cam_Mount_32x32_UVC_Module/) を、この形状に合わせたものです。

## メイン設計との比較
#### 利点:
- SO-101 フォロワーアーム手首の六角ナット埋め込みに合わせてある

#### 欠点:
- 新しいカメラへの合わせ込みは必要
- サーボ付属ではないネジ 2 本とナット 2 個が必要
- SO-100 との後方互換はない

## 必要な部品
### ハードウェア:
- **USB カメラモジュール**（1）— [推奨モデル](https://www.amazon.com/innomaker-Computer-Raspberry-Support-Windows/dp/B0CNCSFQC1/ref=pd_lpo_d_sccl_3/132-7372155-9780230?pd_rd_w=eYz4L&content-id=amzn1.sym.4c8c52db-06f8-4e42-8e56-912796f2ea6c&pf_rd_p=4c8c52db-06f8-4e42-8e56-912796f2ea6c&pf_rd_r=XC3EXZRSSXKDB1G0Z5D7&pd_rd_wg=1wTpn&pd_rd_r=932b1976-9ac7-4cef-9774-f0f9c3acb804&pd_rd_i=B0CNCSFQC1&psc=1)。32mm × 32mm、最低 720p / 30fps の USB カメラモジュールなら他でも使えることが多いです
- [3D プリントした六角ナット埋め込みカメラアダプタ](stl/SO-ARM101_camera_wrist_mount.stl)（1）
   - STL の向きのままツリーサポートで印刷することを推奨。ぐらつきを防ぐためインフィル 40% が目安です
- **M2 ネジ**（4）— Feetech サーボに付属する小さい方のネジです。
- **M3 × 8mm ネジ**（2）
- **M3 六角ナット**（2）

<img height="200" alt="カメラモジュール例" src="https://github.com/user-attachments/assets/18099e1d-754c-4877-871f-9113a0dff062" />

## 組み立て手順
### 手順 1: アダプタをグリッパーに取り付ける

1. **六角ナット埋め込みカメラアダプタ** を 3D プリントする。

2. モーター 6 を外す。[Moving Jaw](../../STL/SO101/Individual/Moving_Jaw_SO101.stl) は付けたままにして構いません。

3. 六角ナットを [Wrist Roll Element](../../STL/SO101/Individual/Wrist_Roll_Follower_SO101.stl) の埋め込み穴に入れる。

<img width="300" src="https://github.com/user-attachments/assets/68f34831-eeb1-4e90-83a2-4e204a94ac51" />

4. モーター 6 を元どおり取り付け、元の M2 ネジで固定する。

5. M3 ネジ 2 本でアダプタを固定する。

### 手順 2: カメラを取り付ける
1. **カメラモジュール** を取り出す。
2. **カメラモジュール** の 4 穴を **カメラアダプタ** に合わせ、**M2 ネジ** 4 本で固定する。

<img width="300" src="https://github.com/user-attachments/assets/baea05ba-2865-4d41-8a53-0ca6ef2b73fe" />

### 手順 3: ソフトウェア設定とフォーカス調整
1. ソフトウェアで解像度と FPS を設定する。
- **注意**: カメラの最大解像度が高くても、解像度は *640 × 480*、FPS は *30* が無難です。多くのモデルは低解像度で学習し、高すぎるとデータが膨らむだけです。
2. アームの電源を入れ、取り付けたカメラの映像を確認する（Mac なら *QuickTime* → *新規ムービー収録* でも見られます）。
- **注意**: フォーカスは手動で、最初はかなりボケて見えます。映像がはっきりするまでレンズを時計回り／反時計回りに回して調整してください。
